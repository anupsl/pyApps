...
class CheckAvailabiltyName():
    @staticmethod
    def checkExists(groupName):
        checkExistsEndpoint = IrisHelper.constructUrl('checkexists').replace('{name}', groupName)
        response = Utils.makeRequest(url=checkExistsEndpoint, data='', auth=IrisHelper.constructAuthenticate(),
    ...
    def assertCheckExists(response, expectedStatusCode, isExists=True, expectedErrorCode=[], expectedErrorMessage=[]):
        if response['constructed'].lower() == 'pass':
            if expectedStatusCode >= 200 and expectedStatusCode <= 300:
                ...
                Assertion.constructAssertion(response['json']['entity'] == isExists,
                                             'isExists value in actual :{} and expected :{}'.format(
                                                 response['json']['entity'], isExists))
