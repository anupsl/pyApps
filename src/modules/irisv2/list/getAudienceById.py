...
class GetAudienceById():

	@staticmethod
	def getById(groupId):
		endpoint = IrisHelper.constructUrl('getbyid').replace('{group_id}', str(groupId))
		response = Utils.makeRequest(url=endpoint, data='', auth=IrisHelper.constructAuthenticate(), headers=IrisHelper.constructHeaders(), method='GET')
