...
class Test_Veneno_RateLimit_EMAIL_ORG():
    ...
    def setup_class(self):
        ...
        self.testObjectForRateLimit = VenenoHelper.preRequisitesForVenenoRateLimit('email')
    ...
    def test_veneno_rateLimit_allStrategySatisfied_email_sanity(self, listType):
        ...
        authorize.assertUserPresenceInNsAdminTable(communicationId , VenenoHelper.getCommunicationBucketId(communicationId), int(json.loads(self.testObjectForRateLimit['list'][listType]['groupVersionDetails']['TEST']['params'])['test_count']),verify=False,channel='EMAIL')
