...
class Test_VenenoThrift_SKIP():
    
    def setup_class(self):
        Logger.logSuiteName(str(self).split('.')[-1])
        self.details = VenenoHelper.preRequisitesForVeneno(testControlType='skip')
    ...
    def test_venenoThrift_skippedUser_CallTask(self):
        ...
        communicationDetailObject = VenenoObject.communicationDetail(cdDetailsBody)
        communicationId = self.connObj.addMessageForRecipients(communicationDetailObject)
        VenenoDBAssertion(cdDetailsBody['campaignId'], cdDetailsBody['communicationType'], communicationId, cdDetailsBody['overallRecipientCount'], cdDetailsBody['recipientListId'], cdDetailsBody['subject'], testControlType='skip', skippedReasons=['Custom Tag Not Present']).check()
    ...
    def est_venenoThrift_inboxUser_MobilePush_Android_Sanity(self):
        ...
        try:
            details = VenenoHelper.preRequisitesForVenenoMobilePush('android',testControlType='skip')
